...
"""
import logging
from lib.python_onvif_zeep.onvif import ONVIFCamera
...
class CameraControl:
    ...
    def __init__(self, ip, user, password,wsdl_path , port):
        self.__cam_ip = ip
        self.__cam_user = user
        self.__cam_password = password
        self.__wsdl_path = wsdl_path
        self.__port = port
    ...
    def camera_start(self,wsdl_path='cfg/'):
        """
        ...
        """
        mycam = ONVIFCamera(self.__cam_ip, self.__port, self.__cam_user, self.__cam_password,wsdl_dir=self.__wsdl_path)
